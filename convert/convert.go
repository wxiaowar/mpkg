...
func StringToInt8(v string) (d int8, err error) {
...
//TODO if need, add string to int16

func StringToInt32(v string) (d int32, err error) {
	tmp, err := strconv.ParseInt(v, 10, 32)
	if err != nil {
		return
	}
...
func Uint64ToString(from uint64) (to string) {
	return strconv.FormatUint(from, 10)
}

func Float64ToString(from float64) string {
	return strconv.FormatFloat(from, 'f', -1, 64)
}
...
func ToTime(v interface{}, layout ...string) (t time.Time, e error) {
	switch value := v.(type) {
	case string:
		l := ptime.TIME_LAYOUT_1
		if len(layout) > 0 {
			l = layout[0]
		}
		return time.ParseInLocation(l, value, ptime.Local)
	case time.Time:
		return value, nil
	default:
		sec, e := ToInt64(value)
